...
function Horw() {
  ...
  useEffect(() => {
    const ambilDataUser = async () => {
      try {
        ...
        setJurusanOptions([...new Set(siswa.map((s) => s.jurusan))]);
        setMapelOptions([...new Set(guru.map((g) => g.mapel))]);
      } catch (error) {
  ...
  const filteredDataTagihan = dataTagihan.filter((item) => {
    const matchName = item.nama?.toLowerCase().includes(searchTermTagihan.toLowerCase());
    const matchStatus =
      ...
      <div className="flex-1 p-8 font-sans">
        <h1 className="text-4xl font-bold text-center text-gray-800 mb-10 flex items-center justify-center gap-2">
         ...
         <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mt-6">
          <div className="bg-white border-t-4 border-sky-600 rounded-lg shadow-md p-6 text-center">
            <i className="ri-database-2-line text-3xl text-sky-600"></i>
            <h3 className="text-xl font-semibold mt-2 text-gray-700">Total Tagihan</h3>
            <p className="text-3xl font-bold my-1">{summary.totalCount}</p>
            <p className="text-lg text-gray-600 font-semibold">{formatRupiah(summary.totalAmount)}</p>
          </div>
          <div className="bg-white border-t-4 border-green-500 rounded-lg shadow-md p-6 text-center">
            <i className="ri-check-double-line text-3xl text-green-500"></i>
            <h3 className="text-xl font-semibold mt-2 text-gray-700">Sudah Lunas</h3>
            <p className="text-3xl font-bold my-1 text-green-600">{summary.sudahBayarCount}</p>
            <p className="text-lg text-green-700 font-semibold">{formatRupiah(summary.sudahBayarAmount)}</p>
          </div>
          <div className="bg-white border-t-4 border-red-500 rounded-lg shadow-md p-6 text-center">
            ...
            <h3 className="text-xl font-semibold mt-2 text-gray-700">Belum Lunas</h3>
            <p className="text-3xl font-bold my-1 text-red-600">{summary.belumBayarCount}</p>
            <p className="text-lg text-red-700 font-semibold">{formatRupiah(summary.belumBayarAmount)}</p>
          </div>
        </div>
              ...
              {filteredDataUser.length > 0 ? (
                filteredDataUser.map((user, index) => (
                  ...
                  >
                    ...
                    <td className="py-3 px-4">{getCategoryIcon(user.kategori)}{user.kategori}</td>
              ...
              {filteredDataTagihan.length > 0 ? (
                filteredDataTagihan.map((item, index) => (
                  ...
                  >
                    ...
                    <td className="py-3 px-4 text-center">{item.jenis_tagihan}</td>
