...
const RekapPresensi = () => {
   ...
   const fetchPresensi = async () => {
    try {
      ...
      const formatted = res.data.map((d) => ({
        ...d,
        tanggalFormatted: formatTanggal(d.tanggal),
      }));
  ...
  useEffect(() => {
    ...
    // Polling: refresh data tiap 5 detik
              ...
              <table className="min-w-full table-auto text-gray-700">
                ...
                <tbody>
                  {dataPresensi.map((item, i) => (
                    <tr key={item.id} className="hover:bg-sky-100">
                      ...
                      <td className="py-3 px-4 text-right">{item.tanggalFormatted}</td>
